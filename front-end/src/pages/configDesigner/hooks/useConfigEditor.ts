...
function serializeItem(item: Record<string, unknown>, indentStr: string): string {
...
function appendItemToYaml(configText: string, key: string, item: Record<string, unknown>): string {
...
function replaceItemInYaml(configText: string, seqKey: string, idKey: string, id: string, item: Record<string, unknown>): string {
    const doc = parseDocument(configText);
    ...
    if (!itemRange) {
        ...
        if (idx !== -1) seq.set(idx, doc.createNode(item));
        return doc.toString();
    }

    // walk back to the start of the '  - ' line and replace up to the item's end
    const lineStart = configText.lastIndexOf('\n', itemRange[0] - 1) + 1;
    return configText.slice(0, lineStart)
        + serializeItem(item, getIndentStr(configText, itemRange[0]))
...
export function useConfigEditor({builtObject, category, config, configText, setConfig, editingEntry, onEditSaved}: ConfigEditorProps) {
